...
def add_proteins_from_file(proteome, filename, delimiter='\t', return_dictionary = True, safe=True, skip_bad=True, verbose=True):
    ...
    One protein defined per line (with NO duplicates allowed - duplicate entries on the file
    will trigger an un-rescuable error) where key:values are optional and can be between 0 and n
    ...
    - If a protein with the UID from the file exists in the passed proteome then this will throw
      an exception unless safe=False 
    ...
    return_dictionary : bool
        If set to true, this function will return the protein dictionary and will NOT add that
        dictionary to the proteome - i.e. the function basically becomes a parser for SHEPHARD-compliant
        protein files. Default = False

    safe : boolean 
        If set to True then any exceptions raised during the protein-adding process are acted
        on. Specifically this becomes relevant if we wish to overwrite duplicates (or throw an exception
        on duplicates).
        Default = True.
    ...
    skip_bad : boolean
        ...
        printed (regardless of verbose flag). skip_bad exclusively influences the file-reading
        part of the process.
    ...
    None or dict
        ...
        value, but the proteins are added to the Proteome object passed as the first argument. If
        return_dictionary is set to True the function returns the parsed proteins dictionary without
        ...
        for protein in proteome:
            ...
            if len(protein.attributes) > 0:
                ...
                for k in protein.attributes:
                    line = line + delimiter +  "%s:%s" %(k, protein.attribute(k)) 
