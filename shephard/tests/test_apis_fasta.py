...
def test_fasta_to_proteome_part_1():
    ...
    P = fasta.fasta_to_proteome(f'{test_data_dir}/testset_1.fasta', proteome=P)
    assert len(P) == 18
...
def test_fasta_to_proteome_part_2():
    ...
    P = fasta.fasta_to_proteome(f'{test_data_dir}/testset_1.fasta', build_unique_ID=header_parser)
    assert len(P) == 9

    # this should trigger an exception because we're adding in duplicates
    with pytest.raises(ProteomeException):
        P = fasta.fasta_to_proteome(f'{test_data_dir}/testset_1.fasta', build_unique_ID=header_parser, proteome=P)
    ...
    P = fasta.fasta_to_proteome(f'{test_data_dir}/testset_1.fasta', build_unique_ID=header_parser, proteome=P, force_overwrite=True)
    assert len(P) == 18

    P = fasta.fasta_to_proteome(f'{test_data_dir}/testset_1.fasta', proteome=P)
    assert len(P) == 27

    P = fasta.fasta_to_proteome(f'{test_data_dir}/testset_1.fasta', proteome=P, use_header_as_unique_ID=True)
    assert len(P) == 36

    # this SHOULD trigger an exception because we shouldn't
    with pytest.raises(APIException):
        P = fasta.fasta_to_proteome(f'{test_data_dir}/testset_1.fasta', proteome=P, use_header_as_unique_ID=True, build_unique_ID=header_parser)
